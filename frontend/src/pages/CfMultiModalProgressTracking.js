...
/**
 * Gap/CFS Feature Page: Multi-modal progress tracking
 * Slug: multi-modal-progress-tracking
 * Backend: /api/cf-multi-modal-progress-tracking
 */
export default function MultiModalProgressTrackingPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-multi-modal-progress-tracking', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Multi-modal progress tracking</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Combine time-lapse photos, GPS, timesheets, sensor data to auto-generate daily reports.</p>
